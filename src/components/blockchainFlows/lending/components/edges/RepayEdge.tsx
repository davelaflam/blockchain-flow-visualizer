...
// Custom edge component for the Repay edge in the lending flow
export default function RepayEdge({
  id,
  sourceX,
  sourceY,
  targetX,
  ...
  // We want a path that goes down from the source, then horizontally to the left,
  // then horizontally to the right, and finally up to the target
  // This creates an S-shaped path that will be distinct from the Withdraw edge
  const verticalOffset = 40; // Vertical distance from the nodes
  const horizontalOffset = 40; // Horizontal distance for the S-curve

  // Create a custom path with an S-curve
  const edgePath = `
    M ${sourceX} ${sourceY}
